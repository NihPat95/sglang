...
def build_aaronson_batch_config(
    ...
    *,
    ...
    for request in requests:
        config = request.watermark
        is_aaronson = (
            isinstance(config, WatermarkRequestConfig) and config.provider == "aaronson"
        )
        keys.append(
            parse_watermark_key(default_key)
            if is_aaronson and default_key is not None
            else 0
        )
        context_windows.append(default_context_window)
        enabled.append(is_aaronson and default_key is not None)
    return (
...
def hash_contexts(
    context_tokens: torch.Tensor, context_lengths: torch.Tensor
) -> torch.Tensor:
    state = torch.zeros(
        context_tokens.shape[0], dtype=torch.int64, device=context_tokens.device
    )
    lengths = context_lengths.to(torch.int64)
    for index in range(context_tokens.shape[1]):
        mixed = _murmur3_mix(state, context_tokens[:, index].to(torch.int64))
        state = torch.where(index < lengths, mixed, state)
    return _fmix32(state ^ (lengths * 4))


def watermark_hash32_torch(
...
def watermark_hash32_pairs_torch(
    keys: torch.Tensor, context_hashes: torch.Tensor, token_ids: torch.Tensor
) -> torch.Tensor:
    state = torch.zeros_like(keys, dtype=torch.int64)
    state = _murmur3_mix(state, keys & _MASK32)
    state = _murmur3_mix(state, (keys >> 32) & _MASK32)
    state = _murmur3_mix(state, context_hashes & _MASK32)
    state = _murmur3_mix(state, token_ids & _MASK32)
    return _fmix32(state ^ 16)


def select_aaronson_tokens_torch(
    probabilities: torch.Tensor,
    context_hashes: torch.Tensor,
    keys: torch.Tensor,
) -> torch.Tensor:
    token_ids = torch.arange(probabilities.shape[-1], device=probabilities.device)
    hashed = watermark_hash32_torch(keys, context_hashes, token_ids)
...
def force_aaronson_tokens(
    *,
    ...
    probabilities = truncate_probabilities(logits, temperatures, top_ks, top_ps, min_ps)
    rows = eligible.nonzero(as_tuple=True)[0]
    if rows.numel() == 0:
        return
    selected = select_aaronson_tokens_torch(
...
class AaronsonWatermarkState:
    def __init__(
        ...
        *,
        ...
        self.default_key = (
            parse_watermark_key(default_key) if default_key is not None else None
        )
    ...
    def create(
        ...
        *,
        ...
        default_key: Optional[str],
        device: str,
    ) -> Optional[AaronsonWatermarkState]:
        if not enabled:
            return None
        return cls(
    ...
    def prompt_tails(self, batch: ScheduleBatch) -> Optional[list[Optional[list[int]]]]:
        ...
        for index, request in enumerate(batch.reqs):
            if index >= num_prefill_reqs:
                ...
                tails.append(list(request.get_fill_ids()[-self.context_window :]))
        return tails

    def init_from_prompt(
        self,
        req_pool_indices: torch.Tensor,
        prompt_tails: Optional[Sequence[Optional[Sequence[int]]]],
    ) -> None:
        if prompt_tails is None:
            return
        assert len(prompt_tails) == req_pool_indices.shape[0]
        valid_positions = [
            index
            for index, token_ids in enumerate(prompt_tails)
        ...
        tails = [list(prompt_tails[index]) for index in valid_positions]
        lengths = torch.tensor(
            [len(token_ids) for token_ids in tails],
            dtype=torch.int32,
            device=device,
        )
        padded = torch.tensor(
    ...
    def _batch_config(
        ...
        assert sampling_info.aaronson_keys is not None
        assert sampling_info.aaronson_context_windows is not None
        assert sampling_info.aaronson_enabled is not None
        return (
            sampling_info.aaronson_keys,
            sampling_info.aaronson_context_windows,
            sampling_info.aaronson_enabled,
        )

    def context_windows(self, sampling_info: SamplingBatchInfo) -> torch.Tensor:
        return self._batch_config(sampling_info)[1]
    ...
    def force_speculative(
        ...
        *,
        ...
        keys, _, watermark_enabled = self._batch_config(sampling_info)
        ...
        eligible = watermark_enabled & (top_ks > 1) & (context_lengths > 0)
        ...
        *,
        ...
        for position in range(accept_tokens.shape[1]):
            ...
            self.append(
                ...
                token_ids=accept_tokens[valid, position],
            )
    ...
    def force(
        ...
        *,
        ...
        keys, context_windows, watermark_enabled = self._batch_config(sampling_info)
        if logits.is_cuda:
            from sglang.kernels.ops.sampling.aaronson_selector import (
                prepare_aaronson_contexts_triton,
            )

            batch_size = req_pool_indices.shape[0]
            context_hashes = self.context_hash_buffer[:batch_size]
            selected = self.eligible_buffer[:batch_size]
            prepare_aaronson_contexts_triton(
                token_ids=self.token_ids,
                lengths=self.lengths,
                write_positions=self.write_positions,
                watermarked_context_hashes=self.watermarked_context_hashes,
                num_watermarked_contexts=self.num_watermarked_contexts,
                req_pool_indices=req_pool_indices,
                context_windows=context_windows,
                watermark_enabled=watermark_enabled,
                top_ks=sampling_info.top_ks,
                output_context_hashes=context_hashes,
                output_eligible=selected,
            )
            force_aaronson_tokens(
                logits=logits,
                context_hashes=context_hashes,
        ...
        context_hashes = hash_contexts(contexts, context_lengths)
        eligible = (
            watermark_enabled & (sampling_info.top_ks > 1) & (context_lengths > 0)
        )
    ...
    def append(
        self, *, req_pool_indices: torch.Tensor, token_ids: torch.Tensor
    ) -> None:
        pool_indices = req_pool_indices.to(torch.int64)
        write_positions = self.write_positions[pool_indices]
