...
@triton.jit
def _aaronson_partial_argmax_kernel(
    probabilities,
    context_hashes,
    keys,
    partial_scores,
...
@triton.jit
def _aaronson_finalize_argmax_kernel(
    partial_scores,
    partial_token_ids,
    output_token_ids,
    num_splits: tl.constexpr,
...
@triton.jit
def _prepare_aaronson_contexts_kernel(
    token_ids,
    lengths,
    write_positions,
...
@triton.jit
def _aaronson_force_partial_argmax_kernel(
    logits,
...
@triton.jit
def _aaronson_finalize_and_write_kernel(
    logits,
    eligible,
    partial_scores,
    partial_token_ids,
...
def select_aaronson_tokens_triton(
    ...
    if not probabilities.is_cuda:
        raise ValueError("Triton Aaronson selection requires CUDA tensors")
    ...
    if not all(
        ...
        raise ValueError("Triton Aaronson inputs must be contiguous CUDA tensors")
    ...
    _aaronson_partial_argmax_kernel[(batch_size, num_splits)](
        probabilities,
        context_hashes,
        keys,
        partial_scores,
    ...
    _aaronson_finalize_argmax_kernel[(batch_size,)](
        partial_scores,
        partial_token_ids,
        output_token_ids,
        num_splits=num_splits,
...
def prepare_aaronson_contexts_triton(
    *,
    ...
    _prepare_aaronson_contexts_kernel[(batch_size,)](
        token_ids,
        lengths,
        write_positions,
        watermarked_context_hashes,
...
def force_aaronson_tokens_triton(
    *,
    logits: torch.Tensor,
    context_hashes: torch.Tensor,
    ...
    _aaronson_force_partial_argmax_kernel[(batch_size, num_splits)](
        logits,
        sorted_probabilities,
        sorted_token_ids,
        cumulative_probabilities,
    ...
    _aaronson_finalize_and_write_kernel[(batch_size,)](
        logits,
        eligible,
        partial_scores,
        partial_token_ids,
